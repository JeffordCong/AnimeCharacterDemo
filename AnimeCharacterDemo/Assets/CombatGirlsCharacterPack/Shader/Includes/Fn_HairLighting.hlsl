...
float3 HairShading(float3 BaseColor, float Specular, float Roughness, float Scatter, float3 N, float3 V, half3 L,
                   float Shadow, float InBacklit, float Area)
{
...
float4 DepthPeelingHairColor(float3 baseColor, float _Specular, float Roughness, float3 positionWS, float3 normalWS,
                             float3 viewWS, float Scatter, float ao, float EnvRotation, float alpha, float alphaClip)
{
    float3 direct = DirectLighting_float(baseColor, _Specular, Roughness, positionWS, normalWS, viewWS,
                                         Scatter);
    float3 indirect = IndirectLighting(baseColor, _Specular, Roughness, positionWS, normalWS, viewWS,
                                       Scatter, ao, EnvRotation);
